...
def fcn_resnet101(outchannels=1, mode_='None', att=False,act_types =['sigmoid'], use_selu=False, alpha=1.0):
    ...
    if att == 'cbam':
        model.classifier[4]=nn.Sequential(
            CBAM(512,act_types=act_types,alpha=alpha),
            OutConv(512,outchannels,mode_=mode_) 
            )
        ...
        if att:        
            ...
            model.classifier[4]= OutConv(512,outchannels,mode_=mode_)
        

    
    if use_selu:
        model.classifier[2]=nn.SELU()    
